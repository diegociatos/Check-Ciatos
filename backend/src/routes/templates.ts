...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { Titulo, Descricao, Responsavel, Prioridade, PontosValor, Recorrencia, DiaSemana, DiaMes, HoraLimite, CriadoPor } = req.body;
    
    const id = uuidv4();
    
    await pool.execute(`
      INSERT INTO task_templates (id, titulo, descricao, responsavel, prioridade, pontos_valor, recorrencia, dia_semana, dia_mes, hora_limite, ativa, criado_por)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE, ?)
    `, [id, Titulo, Descricao || null, Responsavel, Prioridade || 'MEDIA', PontosValor || 10, Recorrencia || 'NENHUMA', DiaSemana || null, DiaMes || null, HoraLimite || null, CriadoPor || null]);
...
router.post('/:id/generate', async (req: Request, res: Response) => {
  try {
    ...
    // Atualizar última execução do template
